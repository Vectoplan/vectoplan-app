# /services/app/routes/ui/crawlab.py
...
bp = Blueprint("ui_crawlab", __name__)
...
def _apply_cache_headers(resp: Response, *, no_store: bool = False) -> None:
    """
    DEV: no-store
    PROD: kurzer Cache für Redirect/JSON vermeiden wir i.d.R. (no-store),
          da Ziel-URL/Session sich ändern können.
    """
    try:
...
def _ensure_conversation(chat_id: str) -> Optional[Conversation]:
    """
    Crawlab ist nicht chat-abhängig, aber wir halten das UI-Namespace konsistent.
...
def _safe_rel_path(raw: str) -> str:
    ...
    try:
        ...
        # absolute URL / schema / protocol-relative blocken
        low = s.lower()
        ...
        # nur relative Segmente
        s = s.lstrip("/")

        # normpath benötigt einen führenden Slash, sonst kann es '.' zurückgeben
...
def _build_crawlab_base() -> str:
    """
    Cached: public base + base path.
    Beispiel:
      CRAWLAB_PUBLIC_URL=http://localhost:8080
      CRAWLAB_BASE_PATH=/
      -> http://localhost:8080/
    """
    try:
        public_base = _norm_url(_cfg_str("CRAWLAB_PUBLIC_URL", "http://localhost:8080"), "http://localhost:8080")
        base_path = _norm_path(_cfg_str("CRAWLAB_BASE_PATH", "/"), "/")
...
def _join_url(base: str, rel: str) -> str:
    ...
    try:
        ...
        if not b:
            return ""
        parts = urlsplit(b)
        # base path sauber zusammenführen
...
def _build_target_url() -> Tuple[str, Dict[str, Any]]:
    ...
    """
    meta: Dict[str, Any] = {"service": "crawlab", "source": "config"}

    base = _build_crawlab_base()
    if not base:
        meta["error"] = "CRAWLAB_PUBLIC_URL not configured"
        return "", meta
...
@bp.get("/ui/chat/<chat_id>/crawlab")
def crawlab_page(chat_id: str):
    """
    Iframe-Ziel für Admin → DataMining.
    Minimal-Variante (Start):
      - validiert chat_id (Konversation existiert)
      - redirect (302) auf CRAWLAB_PUBLIC_URL (+ optionaler Unterpfad)
    ...
    if not target:
        return _simple_html_error(meta.get("error") or "crawlab target not configured", 500)

    # optional: direct=0 liefert eine kleine HTML-Seite statt Redirect (Debug/Fallback)
    if request.args.get("direct") == "0":
        open_label = "Crawlab öffnen"
        html = (
            "<!doctype html><html lang='de'><head>"
            "<meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'>"
            "<title>Crawlab</title></head>"
...
@bp.get("/ui/chat/<chat_id>/crawlab.json")
def crawlab_json(chat_id: str):
    """
